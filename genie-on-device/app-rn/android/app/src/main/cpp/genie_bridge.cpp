...
struct QueryCtx {
  JNIEnv* env = nullptr;
  jobject sink = nullptr;      // com.geniechatrn.genie.TokenSink
...
void throwGenieError(JNIEnv* env, const char* what, Genie_Status_t status) {
  ...
  LOGE("%s", msg.c_str());
  jclass cls = env->FindClass("com/geniechatrn/genie/GenieException");
  if (cls != nullptr) {
...
Java_com_geniechatrn_genie_GenieBridge_nativeCreate(JNIEnv* env,
                                                jclass,
                                                jstring configJson,
...
Java_com_geniechatrn_genie_GenieBridge_nativeQuery(JNIEnv* env,
                                               jclass,
                                               jlong handle,
                                               jstring prompt,
                                               jobject sink) {
...
/**
 * Hard ceiling on GENERATED tokens for subsequent queries.
 *
 * This is the backstop against a model that never emits EOS -- at small context
 * lengths these models do get stuck repeating themselves, and without a cap the
 * only limit is the context window itself, which means a wedged generation
 * burns the whole window before it stops. Genie enforces this itself, so it
 * costs nothing per token.
 */
JNIEXPORT void JNICALL
Java_com_geniechatrn_genie_GenieBridge_nativeSetMaxTokens(JNIEnv* env,
                                                          jclass,
                                                          jlong handle,
                                                          jint maxTokens) {
  auto* session = reinterpret_cast<Session*>(handle);
  if (session == nullptr || session->dialog == nullptr || maxTokens <= 0) return;
  Genie_Status_t status =
      GenieDialog_setMaxNumTokens(session->dialog, static_cast<uint32_t>(maxTokens));
  // Not fatal: an uncapped dialog still works, it just has no runaway guard.
  if (status != GENIE_STATUS_SUCCESS) {
    LOGE("GenieDialog_setMaxNumTokens(%d) failed: %s", maxTokens, statusName(status));
  }
}

JNIEXPORT void JNICALL
Java_com_geniechatrn_genie_GenieBridge_nativeAbort(JNIEnv*, jclass, jlong handle) {
  auto* session = reinterpret_cast<Session*>(handle);
  if (session != nullptr && session->dialog != nullptr) {
...
JNIEXPORT void JNICALL
Java_com_geniechatrn_genie_GenieBridge_nativeReset(JNIEnv* env, jclass, jlong handle) {
  auto* session = reinterpret_cast<Session*>(handle);
...
Java_com_geniechatrn_genie_GenieBridge_nativeContextOccupancy(JNIEnv*, jclass, jlong handle) {
  auto* session = reinterpret_cast<Session*>(handle);
...
JNIEXPORT void JNICALL
Java_com_geniechatrn_genie_GenieBridge_nativeFree(JNIEnv*, jclass, jlong handle) {
  auto* session = reinterpret_cast<Session*>(handle);
